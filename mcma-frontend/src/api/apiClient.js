...
const api = {
  screens: {
    ...
    filterScreen: async (cinemaId) => {
      try {
        const token = localStorage.getItem("token");
        if (!token) {
          throw new Error("No authentication token found");
        }
        const response = await apiClient.get(`/admin/cinema/${cinemaId}/screen/filter`, {
          headers: {
  ...
  schedules: {
    ...
    addSchedules: async (data) => {
      try {
        ...
        const response = await apiClient.post(
          `/admin/cinema/schedule`,
          data, // The payload (body)
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  movies: {
    ...
    filterMovie: async () => {
      try {
        ...
        const response = await apiClient.get("/admin/movie/filter", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  cinemas: {
    filterCinema: async () => {
      try {
        ...
        const response = await apiClient.get("/admin/cinema/filter", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
