...
public interface MovieRepository extends JpaRepository<Movie, Integer> {
    ...
    @Query(nativeQuery = true, value = """
                 SELECT
                        ...
                        m.trailer_link,
                        m.date_publish,
                        GROUP_CONCAT(DISTINCT mrd.name SEPARATOR ',') AS rating_name,\s
                        GROUP_CONCAT(DISTINCT mrd.description SEPARATOR ',') AS rating_description,\s
                        GROUP_CONCAT(DISTINCT mgd.name SEPARATOR ',') AS genre_name,\s
                        GROUP_CONCAT(DISTINCT mpd.name SEPARATOR ',') AS performer_name,\s
                        GROUP_CONCAT(DISTINCT mpd.performer_type SEPARATOR ',') AS performer_type,\s
                        GROUP_CONCAT(DISTINCT mpd.performer_sex SEPARATOR ',') AS performer_sex\s
                 FROM movie m
